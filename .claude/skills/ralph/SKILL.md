...
allowed-tools: "Read Write Edit Glob Grep Bash(pnpm *) Bash(./scripts/*) Bash(./.claude/skills/ralph/scripts/*) Bash(./.claude/skills/full-check/scripts/*) Bash(mkdir *) Bash(cp *) Bash(mv *) Bash(ls *) Bash(git *) Bash(cat *) Bash(echo *) Agent"
---
...
1. 阅读参考文档（CLAUDE.md、ts-dev.md、ts-test.md、模板、已有 slice 代码、OpenAPI schema）
2. FSD 建模（entities 聚合、features slice、widgets 组合、shared/api DTO + Zod schema）
3. 填写 requirement-design.md（领域分析、API 契约、UI 章节若 ui-surface=true）
4. 填写 task-plan.md（按 entities→shared/api→features/model→features/api→features/ui→widgets→app 拆解）
...
前置：阅读 requirement-design.md + task-plan.md + ts-dev.md + ts-test.md + 已有 slice 代码

开发顺序（严格遵守 Red-Green-Refactor + 分层）：
1. entities — 值对象、聚合、不变量 + 纯 Vitest 单测 → git commit
2. shared/api — DTO + Zod schema + mapper（DTO↔Entity）+ 单测 → git commit
3. features/model — hook / store + MSW 单测 → git commit
4. features/api — 用例编排 + MSW 单测 → git commit
5. features/ui — React 组件 + RTL 单测（用户视角查询） → git commit
6. widgets — 页面级组合 + RTL 单测 → git commit
7. app — Next.js 路由装配（仅 Provider/layout，不承载业务） → git commit
8. 四项预飞：pnpm tsc --noEmit && pnpm vitest run && pnpm biome check src tests && ./scripts/entropy-check.sh
   失败则修复后重跑，直到全部通过
9. 更新 task-plan.md + dev-log.md + handoff.md（to: qa, pre-flight: all pass）→ git commit

关键约束：
- TS strict + noUncheckedIndexedAccess（禁裸 @ts-ignore、禁生产 any）
- entities 纯 TS，禁 react/next/zustand/@tanstack/*
- FSD 依赖方向：app → widgets → features → entities ← shared
- 转换链：API Response ↔ DTO (Zod) ↔ Entity ↔ UI Model
- 测试命名：should_xxx_when_yyy
- 命名导出优先（entities/features/widgets 禁 export default）

完成后输出：开发摘要、测试数量、四项验证结果。
```

**主 Agent 验证**：
- 读取 `handoff.md` 检查 status = pending-review, to = qa
- 读取 pre-flight 确认四项检查均 pass
- 运行 `pnpm tsc --noEmit` 做独立类型验证（快速，非全量）
...
3. 独立重跑四项检查（不信任 @dev 的 pre-flight 标记）：
   pnpm tsc --noEmit && pnpm vitest run && pnpm biome check src tests && ./scripts/entropy-check.sh
4. 编写 test-case-design.md（七节：分层测试 + E2E + 代码审查 + 风格检查）
5. 编写 Playwright E2E 关键路径测试（golden path + 1-2 关键分支，≤ 3 条）
6. 代码审查（entities 纯净、值对象不可变、转换链、FSD 依赖方向、UI Model 边界）
